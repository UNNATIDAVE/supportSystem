myApp.controller('singleTicket', ['$routeParams', '$route', '$location', 'ticketService', function($routeParams, $route, $location, ticketService){
	...
	this.username = JSON.parse(localStorage.gteItem('currentUser')).username;
	this.token = JSON.parse(localstorage.gteItem('currentUser')).token;
	...
	ticketService.getSingleTicket({
		...
		main.email = res.data.data.email;
	...
	this.checkAdmin = function(id){
		if(JSON.parse(localStorage.gteItem('currentUser')).username == "admin@supportsystem.com"){
